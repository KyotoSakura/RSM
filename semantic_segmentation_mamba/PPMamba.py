...
class Cross_OSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # out proj =======================================
        self.out1_proj = nn.Linear(d_inner, d_model, bias=bias, **factory_kwargs)
        self.out2_proj = nn.Linear(d_inner, d_model, bias=bias, **factory_kwargs)
        ...
        self.Ds1 = self.D_init(d_inner, copies=k_group, merge=True) # (K * D)
    ...
    def forward_corev2(self, x1: torch.Tensor, x2: torch.Tensor, channel_first=False, SelectiveScan=SelectiveScanOflex, cross_selective_scan=crossmodal_selective_scan, force_fp32=None):
        if not channel_first:
            x1 = x1.permute(0, 3, 1, 2).contiguous()
            x2 = x2.permute(0, 3, 1, 2).contiguous()
        # ZSJ V2版本使用的mamba，要改扫描方向在这里改
        x1, x2 = cross_selective_scan(
            x1, x2, self.x1_proj_weight, self.x2_proj_weight, None, None, 
            self.dt1_projs_weight, self.dt2_projs_weight, self.dt1_projs_bias, self.dt2_projs_bias,
            self.A1_logs, self.A2_logs, self.Ds1, self.Ds2, delta_softplus=True,
            out_norm=getattr(self, "out_norm", None),
            out_norm_shape=getattr(self, "out_norm_shape", "v0"),
            force_fp32=force_fp32,
            SelectiveScan=SelectiveScan,
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor, **kwargs):
        ...
        if with_dconv:
            x1 = x1.permute(0, 3, 1, 2).contiguous()
            x2 = x2.permute(0, 3, 1, 2).contiguous()
            x1 = self.conv2d1(x1) # (b, d, h, w)
            x2 = self.conv2d2(x2) # (b, d, h, w)
        x1 = self.act(x1)
        x2 = self.act(x2)
        y1, y2 = self.forward_core(x1, x2, channel_first=with_dconv)
        if not self.disable_z:
            y1 = y1 * z1
            y2 = y2 * z2
        out1 = self.dropout1(self.out1_proj(y1))
        out2 = self.dropout2(self.out2_proj(y2))
        return out1, out2
...
class OSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.ssm_branch:
            ...
            self.norms = nn.ModuleList([ norm_layer(hidden_dim // 2)
            for i in range(2)])
            self.ops = nn.ModuleList([ _OSSM(
            ...
            nn.ReLU()), # conv33conv33conv11

            nn.Sequential(
            nn.BatchNorm2d(hidden_dim//2),
            nn.Conv2d(in_channels=hidden_dim//2,out_channels=hidden_dim//2,kernel_size=5,stride=1,padding=2),
            nn.BatchNorm2d(hidden_dim//2),
            nn.ReLU(),
            nn.Conv2d(in_channels=hidden_dim//2, out_channels=hidden_dim//2, kernel_size=1, stride=1),
            nn.ReLU())  # conv55conv55conv11
            ])
    ...
    def global_process2(self, input: torch.Tensor):
        if self.ssm_branch:
            x = input + self.drop_path(self.op(self.norm(input)))
        return x
    
    def local_process(self, input1: torch.Tensor, input2: torch.Tensor):
        if self.ssm_branch:
            input1 = input1.permute(0,3,1,2).contiguous()
            input2 = input2.permute(0,3,1,2).contiguous()
            x1 = self.convds[0](input1)
            x2 = self.convds[1](input2)
            x = torch.cat((x1,x2),dim=1)
            x = x.permute(0,2,3,1).contiguous() # B H W C
        return x
    
    def _forward(self, input: torch.Tensor):
        input_head1, input_head2 = input.chunk(2,dim=-1)
        output = self.local_process(input_head1, input_head2)
        output = self.global_process2(output)
        if self.mlp_branch:
            output = output + self.drop_path(self.mlp(self.norm2(output))) # FFN
        return output + input
...
class PPMamba(nn.Module):
    ...
    def forward(self, x1: torch.Tensor):
        x1 = self.patch_embed(x1) # 64*64*96

        x1_1 = self.encoder_block1(x1) # 64*64*96
        x1_2 = self.encoder_block2(x1_1) # 32*32*192
        x1_3 = self.encoder_block3(x1_2) # 16*16*384
        x1_4 = self.encoder_block4(x1_3)  # 8*8*768
